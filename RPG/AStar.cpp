...
void Astar::InitAstar(int** _maze,int nRow,int nCol,vector<CVec2> vecPos)
{
	maze = _maze;
	Row = nRow;
	Col = nCol;
...
bool Astar::isCanreach(const Point* point, const Point* target, bool isIgnoreCorner) const
{
	...
	{
		if (target->x == vecPos[i].x && target->y == vecPos[i].y)
		{
			isNpc = true;
		}
	}

	if (target->x<0 || target->x> Row - 1
		|| target->y<0 || target->y>Col - 1
		|| maze[target->x][target->y] == 1
		|| target->x == point->x && target->y == point->y
		|| isInList(closeList, target)
		|| isNpc)
		//如果点与当前节点重合、超出地图、是障碍物、是npc或者在关闭列表中，返回false
		return false;
	else
	{
